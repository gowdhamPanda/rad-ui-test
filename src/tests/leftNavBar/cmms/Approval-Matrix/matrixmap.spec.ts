...
test("Edit Approval Matrix Map", async ({ page }) => {
  await doLogin(page);
  await page.goto(`${getBaseUrl()}${routeslist.APPROVAL_MATRIX_MAP}`);
  await page.locator(approval_matrix_map_Variables.PaginationLocator).click()
  ...
  await page.locator("#screencontainer > div > app-approval-matrix-map-dashboard > div > div > nz-card > div.ant-card-body > nz-table > nz-spin > div > div > nz-table-inner-default > div > table > tbody > tr:nth-child(19) > td:nth-child(5) > nz-space > div:nth-child(1) > nz-button-group > button > span > svg").click();
...
test("Testing Alert while updating Part Type", async ({ page }) => {
  ...
  await page.getByText(approval_matrix_map_Variables.EditText).click();
      ...
      // await page.locator(approval_matrix_map_Variables.ApproverOneLocator).click();
      // await page.keyboard.press(approval_matrix_map_Variables.Enter);
        await page.locator("#screencontainer > div > app-approval-matrix-map-createmanage > div > div > nz-card > div.ant-card-body > div > div > div:nth-child(2) > nz-table > nz-spin > div > div > nz-table-inner-default > div > table > tbody > tr.ant-form.ant-form-horizontal.ant-table-row.ng-star-inserted.ng-dirty.ng-touched.ng-valid > td:nth-child(3) > nz-form-item > nz-form-control > div > div > nz-select > nz-select-clear > span > svg > path").click();
        await expect(page.locator(approval_matrix_map_Variables.ApproverOneAlertLocator)).toBeVisible();
        await delay(3000);
      // await page.locator(approval_matrix_map_Variables.ApproverTwoCancelLocator).click();
      // await expect(page.locator(approval_matrix_map_Variables.ApproverTwoAlertLocator)).toBeVisible();
